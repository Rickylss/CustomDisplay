...
|*  and / or non-disclosure agreement.  Otherwise, you have no rights to use or
access this software in any manner.                   *|
|* *|
...
//		   This program applies a fixed custom resolution to the
// attached monitor
//         It applies the custom settings each active display in the following
//         sequence:
...
//
// @assumptions: This code is designed for Win7+ operating systems. It assumes
// that
...
NvAPI_Status ApplyCustomDisplay(int width, int height, bool revert);
...
int main(int argc, char *argv[]) {
  ...
  if (argc < 2) {
    printf("CustomTiming.exe [width]x[height] [--revert]\n");
    printf("    example: CustomTiming.exe 3000x2000\n");
    printf("    example: CustomTiming.exe 3000x2000 --revert\n");
    return 0;
  }
  ...
  printf("Applying custom display settings for %dx%d\n", width, height);

  if (argc >= 3 && strcmp(argv[2], "--revert") == 0) {
    printf("Reverting back to default display settings\n");
    revert = true;
  }
  ...
  ret = ApplyCustomDisplay(width, height, revert);
  if (ret != NVAPI_OK) {
    getchar();
...
NvAPI_Status GetConnectedDisplays(NvU32 *displayIds, NvU32 *noDisplays) {
  ...
  for (NvU32 Count = 0; Count < gpuCount;
       Count++)  // iterating per Physical GPU Handle call
  {
    NvU32 dispIdCount = 0;
    // First call to get the no. of displays connected by passing NULL
    if (NvAPI_GPU_GetConnectedDisplayIds(nvGPUHandle[Count], NULL, &dispIdCount,
                                         0) != NVAPI_OK) {
      return NVAPI_ERROR;
    }

    if (dispIdCount > 0)  // If no. of displays connected > 0 we can proceed to
                          // check if its active
    {
      // alocations for the display ids
      NV_GPU_DISPLAYIDS *dispIds =
          (NV_GPU_DISPLAYIDS *)malloc(sizeof(NV_GPU_DISPLAYIDS) * dispIdCount);

      for (NvU32 dispIndex = 0; dispIndex < dispIdCount; dispIndex++)
        dispIds[dispIndex].version =
            NV_GPU_DISPLAYIDS_VER;  // adding the correct version information

      // second call to get the display ids
      if (NvAPI_GPU_GetConnectedDisplayIds(nvGPUHandle[Count], dispIds,
                                           &dispIdCount, 0) != NVAPI_OK) {
        return NVAPI_ERROR;
      }
...
NvAPI_Status ApplyCustomDisplay(int width, int height, bool revert) {
  ...
  for (NvU32 count = 0; count < noDisplays; count++) {
    ...
    cd[count].width = width;
    cd[count].height = height;
  ...
  ret = NvAPI_DISP_TryCustomDisplay(&displayIds[0], noDisplays,
                                    &cd[0]);  // trying to set custom display
  if (ret != NVAPI_OK) {
    printf("\nNvAPI_DISP_TryCustomDisplay() failed = %d",
           ret);  // failed to set custom display
    return ret;
  } else
    printf(".....Success!\n");
  Sleep(5000);
  ...
  if (ret != NVAPI_OK) {
    printf("\nNvAPI_DISP_SaveCustomDisplay() failed = %d",
           ret);  // failed to save custom display
    return ret;
  } else
    printf(".....Success!\n");
  ...
  if (ret != NVAPI_OK) {
    printf("NvAPI_DISP_RevertCustomDisplayTrial() failed = %d",
           ret);  // failed to revert custom display trail
    return ret;
  } else {
    printf(".....Success!");
    Sleep(5000);
